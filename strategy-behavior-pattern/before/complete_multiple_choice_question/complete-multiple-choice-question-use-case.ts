...
import { IMultipleChoiceQuestionRepo } from "../../multiple_choice_question/config/i-multiple-choice-question.repo"
...
import { ICompletedMultipleChoiceQuestionRepo } from "../config/i-completed-multiple-choice-question.repo"
import {
  CompleteMultipleChoiceQuestionArgs,
  CompleteMultipleChoiceQuestionResult,
  ICompleteMultipleChoiceQuestionUseCase,
} from "./i-complete-multiple-choice-question-use-case"

@Injectable()
export class CompleteMultipleChoiceQuestionUseCase
  implements ICompleteMultipleChoiceQuestionUseCase
{
  constructor(
    @Inject(Repos.completedMultipleChoiceQuestion)
    private readonly completedMultipleChoiceQuestionRepo: ICompletedMultipleChoiceQuestionRepo,
    ...
    private readonly multipleChoiceQuestionRepo: IMultipleChoiceQuestionRepo,
  ...
  async execute(
    ...
    const existingSubQuiz = await this.multipleChoiceQuestionRepo.get(
      args.questionId,
    )
    ...
    const existingCompletedSubQuiz =
      await this.completedMultipleChoiceQuestionRepo.get({
        pathId: args.pathId,
    ...
    const isTheRightAnswer = _.isEqual(
      _.sortedUniq(args.studentResponse),
      _.sortedUniq(existingSubQuiz.idealOptions),
    )
    ...
    await this.completedSubQuizRepo.save({
      ...
      score: reward,
      studentId: args.studentId,
      subQuizType: TSubQuiz.multipleChoiceQuestion,
      updatedAt: completedSubQuizCreatedAt,
    })

    const createdAt = new Date()
    await this.completedMultipleChoiceQuestionRepo.save({
      createdAt,
      pathId: args.pathId,
      questionId: existingSubQuiz.questionId,
      quizId: args.quizId,
    ...
    return Right.create(
      isTheRightAnswer
        ...
        : {
            expectedResponse: existingSubQuiz.idealOptions,
